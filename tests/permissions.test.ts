...
import { accounts1, accounts2, catalog, ctx, exams1, makeTicket, manager, otherStudent, student } from "./fixtures";

describe("permissions", () => {
  const ticket = makeTicket();

  it("a student can view only their own ticket", () => {
    expect(canView(student, ticket, catalog)).toBe(true);
    expect(canView(otherStudent, ticket, catalog)).toBe(false);
  });
  ...
  it("a student can never post an internal note", () => {
    ...
    expect(availableActions(student, ticket, catalog).has("comment_internal")).toBe(false);
  });

  it("staff see their team's queue but not other teams' tickets", () => {
    expect(canView(accounts2, ticket, catalog)).toBe(true);
    expect(canView(exams1, ticket, catalog)).toBe(false);
  });

  it("staff can't pick up another team's ticket", () => {
    const queued = makeTicket({ status: "NEW", assigneeId: null });
    expect(canPickUp(exams1, queued, catalog)).toBe(false);
    expect(() => pickUp(queued, 1, ctx(exams1))).toThrow(/own team/);
    expect(canPickUp(accounts2, queued, catalog)).toBe(true);
  });
  ...
  it("staff can't act on a ticket they don't own, even in their team", () => {
    ...
    expect(availableActions(accounts2, ticket, catalog).size).toBe(0);
  });

  it("a manager can view and work any ticket", () => {
    expect(canView(manager, ticket, catalog)).toBe(true);
  ...
  it("offers owners only the actions their status allows", () => {
    const acts = availableActions(accounts1, ticket, catalog);
